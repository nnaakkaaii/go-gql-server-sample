...
func TransfersByStationCD(ctx context.Context, db DB, stationCD int) ([]*Transfer, error) {
	...
	for rows.Next() {
		var t Transfer
		// scan
		if err := rows.Scan(&t.StationCd, &t.LineCd, &t.LineName, &t.StationName, &t.StationGCd, &t.Address, &t.TransferLineCd, &t.TransferLineName, &t.TransferStationCd, &t.TransferStationName, &t.TransferAddress); err != nil {
			return nil, logerror(err)
		}
		res = append(res, &t)
	}
