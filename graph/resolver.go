...
func init() {
	conn, err := sql.Open("postgres", "host=db port=5432 user=root password=postgres dbname=root sslmode=disable")
	if err != nil {
		panic(err)
	}
	db = conn
...
func (r *Resolver) getStationByName(ctx context.Context, name *string) ([]*model.Station, error) {
	sts, err := models.StationByNamesByStationName(ctx, db, *name)
	if err != nil {
		return nil, err
	}
...
func (r *Resolver) beforeStation(ctx context.Context, obj *model.Station) (*model.Station, error) {
	stationCD := obj.StationCd
	records, err := models.BeforesByStationCD(ctx, db, stationCD)
	if err != nil {
		return nil, err
	}
...
func (r *Resolver) afterStation(ctx context.Context, obj *model.Station) (*model.Station, error) {
	stationCD := obj.StationCd
	records, err := models.AftersByStationCD(ctx, db, stationCD)
	if err != nil {
		return nil, err
	}
